...
from app.core.exceptions import IntentClassificationException, RetrieverException, LLMUnavailableException
...
@pytest.mark.asyncio
async def test_answer_routes_to_crag_on_policy_intent(mock_retriever, mock_llm, mock_intent_classifier, chat_request, mocker):
    mock_intent_classifier.classify.return_value = "policy"
    rag_service = RAGService(mock_retriever, mock_llm, mock_intent_classifier)
    
    rag_service._run_crag_pipeline = AsyncMock(return_value=ChatResponse(response="crag response", request_id="req-1", conversation_id="conv-1"))
...
@pytest.mark.asyncio
async def test_answer_routes_to_agentic_on_user_data_intent(mock_retriever, mock_llm, mock_intent_classifier, chat_request, mocker):
    mock_intent_classifier.classify.return_value = "user_data"
    rag_service = RAGService(mock_retriever, mock_llm, mock_intent_classifier)
    
    rag_service._run_crag_pipeline = AsyncMock()
    rag_service._run_agentic_pipeline = AsyncMock(return_value=ChatResponse(response="agent response", request_id="req-1", conversation_id="conv-1"))
...
@pytest.mark.asyncio
async def test_answer_defaults_to_policy_on_classification_failure(mock_retriever, mock_llm, mock_intent_classifier, chat_request, mocker):
    mock_intent_classifier.classify.side_effect = IntentClassificationException(message="fail", detail="fail")
    rag_service = RAGService(mock_retriever, mock_llm, mock_intent_classifier)
...
@pytest.mark.asyncio
async def test_answer_returns_chat_response(mock_retriever, mock_llm, mock_intent_classifier, chat_request, mocker):
    mock_intent_classifier.classify.return_value = "policy"
    rag_service = RAGService(mock_retriever, mock_llm, mock_intent_classifier)
...
@pytest.mark.asyncio
async def test_answer_propagates_retriever_exception(mock_retriever, mock_llm, mock_intent_classifier, chat_request, mocker):
    mock_intent_classifier.classify.return_value = "policy"
    rag_service = RAGService(mock_retriever, mock_llm, mock_intent_classifier)
...
@pytest.mark.asyncio
async def test_answer_propagates_llm_exception(mock_retriever, mock_llm, mock_intent_classifier, chat_request, mocker):
    mock_intent_classifier.classify.return_value = "policy"
    rag_service = RAGService(mock_retriever, mock_llm, mock_intent_classifier)
