...
from app.core.exceptions import DocumentNotFoundException, LLMUnavailableException
...
async def test_chat_returns_200(async_client, mocker):
    ...
    mock_rag.answer = AsyncMock(
        return_value=ChatResponse(
            response="test response", conversation_id="conv-1", request_id="req-1"
        )
    )
...
async def test_chat_propagates_request_id(async_client, mocker):
    ...
    mock_rag.answer = AsyncMock(
        return_value=ChatResponse(
            response="test response", conversation_id="conv-1", request_id="test-req-id"
        )
    )
